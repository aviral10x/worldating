...
export default function MessagesContent() {
  ...
  const resolveCurrentUser = useCallback(async () => {
    try {
      ...
      const worldAddrRaw =
        typeof window !== "undefined"
          ? localStorage.getItem("world_address")
          : null;
      const lsId =
        typeof window !== "undefined"
      ...
      if (worldAddrRaw && worldAddrRaw.trim()) {
        ...
        const meRes = await fetch(
          `/api/users/by-world?address=${encodeURIComponent(address)}`,
          { headers }
        );
        if (meRes.ok) {
  ...
  const loadConversations = useCallback(async () => {
    try {
      ...
      const data: Conversation[] = (Array.isArray(raw) ? raw : []).map(
        (c: any) => ({
      ...
      const filtered = (data || []).filter((c) =>
        isVerifiedName(c.otherUser.name)
      );
  ...
  const loadMatches = useCallback(async () => {
    try {
      ...
      if (selfAddress) {
        const resByAddr = await fetch(
          `/api/matches?address=${encodeURIComponent(selfAddress)}`,
          { headers }
        );
        if (resByAddr.ok) {
          ...
          if (
            resByAddr.status === 404 &&
            maybeJson?.code === "USER_NOT_FOUND_BY_ADDRESS" &&
            userId
          ) {
            const resById = await fetch(`/api/matches?userId=${userId}`, {
              headers,
            });
            if (resById.ok) {
      ...
      const mapped: Match[] = (Array.isArray(data) ? data : []).map(
        (m: any) => ({
          id: Number(m.id),
          name: m.name,
  ...
  const loadMessages = useCallback(
    async (conv: Conversation, markRead = true) => {
      try {
        ...
        const raw: any[] = await res.json();
        const data: Message[] = (Array.isArray(raw) ? raw : []).map(
          (m: any) => ({
            id: Number(m.id),
  ...
  const otherUserId =
    selected?.otherUser.id != null ? Number(selected.otherUser.id) : null;
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (!res.ok) {
            ...
            toast.message(
              `Waiting for ${match.name} to match back to start chat`
            );
            // Still refresh conversations and try selecting preferred
            const convRes = await fetch(`/api/conversations?userId=${userId}`, {
              headers,
            });
            if (convRes.ok) {
              const fresh: Conversation[] = await convRes.json();
              const onlyVerified = (fresh || []).filter((c) =>
                isVerifiedName(c.otherUser.name)
              );
              ...
              const pref =
                ...
                onlyVerified.find((c) =>
                  c.otherUser.name?.toLowerCase().includes("aviral")
                ) ||
                onlyVerified.find((c) =>
                  c.otherUser.name?.toLowerCase().includes("lia")
                ) ||
                onlyVerified[0] ||
                null;
        ...
        if (convRes.ok) {
          ...
          setConversations(onlyVerified);
          const found =
            onlyVerified.find((c) => c.otherUser.id === match.id) || null;
    ...
    try {
      ...
      const list: Match[] = (Array.isArray(data) ? data : [])
        ...
        .filter((u: any) => {
          ...
          if (myAddr && (u.worldAddress || "").toLowerCase() === myAddr)
            return false;
          return true;
        })
        .map((u: any) => ({
  ...
  const ensureMutualLikeAndStart = useCallback(
    async (target: Match) => {
      if (!userId) return;
      try {
  ...
  useEffect(() => {
    ...
    handleStartChat,
    loadConversations,
    fetchVerifiedTargets,
    ensureMutualLikeAndStart,
  ]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
      <div className="lg:col-span-4">
        <div className="soft-card overflow-hidden">
          ...
          <ul className="p-2 max-h-[40vh] overflow-auto">
            {loadingConvos && (
              <li className="p-3 text-sm text-[var(--muted-foreground)]">
                Loading…
              </li>
            )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {c.unreadCount > 0 && (
                        <span className="ml-2 text-xs px-2 py-0.5 rounded-full bg-[var(--primary)] text-[var(--primary-foreground)]">
                          {c.unreadCount}
                        </span>
                      )}
                    </div>
          ...
          <ul className="p-2 max-h-[30vh] overflow-auto">
            ...
            {unmatchedList.map((m) => (
              <li
                key={m.id}
                className="flex items-center gap-3 p-2 rounded-xl hover:bg-[var(--secondary)]"
              >
                <div className="h-8 w-8 rounded-lg overflow-hidden bg-[var(--secondary)]">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={m.avatarUrl || "/vercel.svg"}
                    alt={m.name}
                    className="h-full w-full object-cover"
                    onError={(e) => {
      ...
      <div className="lg:col-span-8">
        <div className="soft-card h-[70vh] flex flex-col overflow-hidden">
            ...
            {selected ? (
              <div className="flex items-center gap-3">
                <div className="h-8 w-8 rounded-lg overflow-hidden bg-[var(--secondary)]">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={selected.otherUser.avatarUrl || "/vercel.svg"}
          ...
          <div className="flex-1 overflow-auto p-4 space-y-2 bg-[var(--card)]">
            ...
            {messages.map((m) => {
              ...
              return (
                <div
                  key={m.id}
                  className={`max-w-[80%] rounded-xl px-3 py-2 text-sm ${mine ? "ml-auto bg-[var(--primary)] text-[var(--primary-foreground)]" : "bg-[var(--secondary)]"}`}
                >
                  <div>{m.body}</div>
                  <div
                    className={`mt-1 text-[10px] ${mine ? "opacity-80" : "text-[var(--muted-foreground)]"}`}
                  >
          ...
          <div className="p-1 border-t border-[var(--border)] flex items-center gap-2">
            <input
              className="flex-1 h-10 px-1 rounded-lg border bg-transparent"
              placeholder={
                selected
                  ? `Message ${selected.otherUser.name}`
                  : "Select a chat"
              }
              value={input}
